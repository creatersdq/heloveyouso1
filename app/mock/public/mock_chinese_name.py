...
class RandomUtil:
    ...
    # 2 名字（这里只用几个名字 做演示即可）
    NAME_MING = ['壮', '昱杰', '开虎', '凯信', '永斌', '方洲', '长发', '可人', '天弘', '炫锐', '富明', '俊枫', '小玉', '蓝', '琬郡', '琛青', '予舴',
                 '妙妙', '梓茵', '海蓉', '语娜', '馨琦', '晓馥', '佳翊']

    def random_name_str(self, is_two_xing=False):
        """
        生成随机姓名
        :param gender: 性别（男、女）
        ...
        # step2 生成名
        ming = ''
        ming = self.NAME_MING[random.randint(0, len(self.NAME_MING) - 1)]

        return xing + ming


RandomChineseName = RandomUtil()
