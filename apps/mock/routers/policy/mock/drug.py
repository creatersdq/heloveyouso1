...
from apps.public.policy.mock.ypb import Ypb
from apps.public.policy.mock.common import data_base
from apps.public.policy.mock.api import ApiBase
from apps.schems.policy.mock import Mock_Detail, Switch, MockClaimData
...
async def policy_mock(items: Mock_Detail) -> res_wrapper:
    ...
    if items.switchConfig.orderAttachmentDataMock == Switch.on:
        try:
            # 若传入billNo，则以入参为准
            if items.claimDetail.billNoMock is not None:
                await p.order_attachments_mock(apps.gl_session, items.claimDetail.billNoMock)
            else:
                await p.order_attachments_mock(apps.gl_session)
        except Exception as e:
